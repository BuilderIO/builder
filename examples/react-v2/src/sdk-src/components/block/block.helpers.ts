...
export const getRepeatItemData = ({
  ...
  console.log('repeat', { collection: repeat.collection, itemsArray, context });
...
export const shouldPassLinkComponent = (block: RegisteredComponent | null | undefined) => {
  return (
    block && ['Core:Button', 'Symbol', 'Columns', 'Form:Form', 'Builder: Tabs'].includes(block.name)
  );
};
export const shouldPassRegisteredComponents = (block: RegisteredComponent | null | undefined) => {
  return block && ['Symbol', 'Columns', 'Form:Form', 'Builder: Tabs'].includes(block.name);
};
