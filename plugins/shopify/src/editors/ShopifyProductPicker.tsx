...
export class ShopifyProductPicker extends SafeComponent<ShopifyProductPickerProps> {
  ...
  @computed get productInfoCacheValue() {
    ...
    return this.props.context.httpCache.get(
      `${appState.config.apiRoot()}/api/v1/shopify/products/${this.productId}.json?apiKey=${this.props.context.user.apiKey}`
    );
  }
  ...
  getRequestObject(productId: string) {
    ...
    return {
      ...
      request: {
        url: `${appState.config.apiRoot() }/api/v1/shopify/products/{{this.options.product}}.json?apiKey=${this.props.context.user.apiKey}`,
      },
      options: {
        product: productId,
      },
