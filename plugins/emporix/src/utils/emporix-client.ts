...
export class AnonymousTokenHelper {
  ...
  async authenticate() {
    const response = await fetch(`https://api.emporix.io/customerlogin/auth/anonymous/login?client_id=${this._clientId}&tenant=${this._tenant}`)
...
export class EmporixClient {
  ...
  async searchProducts(search: string) {
    const response = await this._executeRequest(`https://api.emporix.io/product/${this._tenant}/products?q=name:~${search}`, {})
    return await response.json();
  }

  async getProduct(id: string) {
    const response = await this._executeRequest(`https://api.emporix.io/product/${this._tenant}/products/${id}`, {})
    return await response.json();
  }
