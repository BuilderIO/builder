...
const things = serverNames.map((packageName, i) => {
  const isReactNative = packageName === 'react-native-74' || packageName === 'react-native-76-fabric';
  const port = 1111 + i;
...
export default defineConfig({
  ...
  webServer: things
    .map(({ packageName, port, portFlag }) => ({
      ...
      ...(packageName === 'react-native-74' || packageName === 'react-native-76-fabric' ? { timeout: 120 * 1000 } : {}),
    }))
    .concat([
      {
