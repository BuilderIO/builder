...
test.describe('Editable Regions in Custom Components', () => {
  ...
  test('Display content for the clicked tab and hide the other', async ({ page, packageName }) => {
    ...
    await verifyTabContent(page, 'Tab 1', 'component.options.tabList.0.children', 'component.options.tabList.1.children');

    await verifyTabContent(page, 'Tab 2', 'component.options.tabList.1.children', 'component.options.tabList.0.children');
  });
