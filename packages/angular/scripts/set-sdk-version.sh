...
LOCAL_DIR=$(pwd)
if [[ "$LOCAL_DIR" =~ .*packages/angular*$ ]]; then
  echo "Setting SDK Version for Angular Gen1 SDK in '$LOCAL_DIR'."
else
  echo "Must run this script from the root of the Angular Gen1 SDK directory, e.g. 'packages/angular'."
