...
function getReactNativeBlockStyles({
  ...
  const largeAndMediumStyles = removeDisplayNone(__spreadValues(__spreadValues({}, responsiveStyles.large || {}), responsiveStyles.medium || {}));
  const styles = __spreadValues(__spreadValues(__spreadValues(__spreadValues({}, context.inheritedStyles), largeAndMediumStyles), responsiveStyles.small || {}), blockStyles);
  const cleanedCSS = cleanCssStyleProps(styles);
  const newStyles = cssToStyleSheet(Object.entries(cleanedCSS));
  return newStyles;
}
export { cleanCssStyleProps, getReactNativeBlockStyles }